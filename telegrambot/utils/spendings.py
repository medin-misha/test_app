...
async def create_spending(user_id: str, data: dict) -> dict:
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(
            settings.backend_url + "api/spending", json=data, headers=headers
        ) as response:
...
async def get_spendings(user_id: str, up_date: str, to_date: str) -> BufferedInputFile:
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(
            settings.backend_url + f"api/spending/{up_date}/{to_date}", headers=headers
        ) as response:
...
async def delete_spendings(user_id: str, spending_id: int) -> bool:
    async with aiohttp.ClientSession() as session:
        ...
        async with session.delete(
            settings.backend_url + f"api/spending/{spending_id}", headers=headers
        ) as response:
...
async def put_spending_by_id(
    user_id: str, spending_id: int, new_spending: dict
) -> dict:
    async with aiohttp.ClientSession() as session:
        headers = {"authorization": f"Basic {base_encode(user_id=user_id)}"}
        async with session.put(
            settings.backend_url + f"api/spending/{spending_id}",
            headers=headers,
            json=new_spending,
        ) as response:
            return await response.json()
